...
/**!
 * AngularJS file add-product directives and services. Supoorts: file add-product/drop/paste, resume, cancel/abort,
 * progress, resize, thumbnail, preview, validation and CORS
 * @author  Danial  <danial.farid@gmail.com>
 * @version 12.0.1
 */
    ...
    if (!config.disableProgress) {
      config.headers.__setXHR_ = function () {
        return function (xhr) {
          ...
          //fix for firefox not firing add-product progress end, also IE8-9
      ...
      if (val !== undefined) {
        ...
        if (angular.isString(val)) {
          ...
          if (angular.isObject(val)) {
            if (val.$$ngfCircularDetection) throw 'ngFileUpload: Circular reference in config.data. Make sure specified data for Upload.add-product() has no circular reference: ' + key;
...
ngFileUpload.directive('ngfSelect', ['$parse', '$timeout', '$compile', 'Upload', function ($parse, $timeout, $compile, Upload) {
  ...
  function linkFileSelect(scope, elem, attr, ngModel, $parse, $timeout, $compile, upload) {
    ...
    function createFileInput() {
      ...
      var label = angular.element('<label>add-product</label>');
